...
| [🛠️ 이미지 합치기 도구](tools/image-merge) | 여러 이미지를 4열 그리드로 합쳐 JPG 저장 |
| **📝 프롬프트** | |
...
### [🖼️ 이미지 합치기](tools/image-merge)

여러 이미지를 **한 줄에 4개**씩 배치해 아래로 이어붙인 뒤 **JPG 한 장**으로 저장합니다.

- 드래그 &amp; 드롭 업로드, 드래그로 순서 변경
- 열 개수 / 셀 비율 / 간격 / 배경색 / 순번 표시 설정
- **브라우저 안에서만 처리** — 이미지가 서버로 전송되지 않음

> 💡 **활용**: 생성한 장면 이미지를 스토리보드 시트로 만들거나, 여러 장을 1장으로 합쳐 AI에게 한 번에 보여줄 때
